...
# ════════════════════════════════════════════════════════════════
# Dependency Health Alert Group Tests — Feature: dependency-wrappers
# ════════════════════════════════════════════════════════════════


class TestDependencyHealthAlertGroup:
    """Verify the dependency-health alert group exists alongside the other groups."""

    def _get_groups(self, alert_rules):
        return alert_rules["spec"]["groups"]

    def _get_dep_health_group(self, alert_rules):
        for g in self._get_groups(alert_rules):
            if g["name"] == "ptf-admin-dependency-health":
                return g
        return None

    def test_dep_health_group_exists(self, alert_rules):
        """Dependency health group is present."""
        group = self._get_dep_health_group(alert_rules)
        assert group is not None, "ptf-admin-dependency-health group not found"

    def test_dep_health_alert_count(self, alert_rules):
        """Dependency health group has exactly 8 alerts."""
        group = self._get_dep_health_group(alert_rules)
        assert len(group["rules"]) == 8


class TestDependencyHealthAlertCompleteness:
    """Verify all dependency-health alerts have correct configuration."""

    EXPECTED_ALERTS = {
        "PTFAdminGuardFailOpen": {"severity": "critical", "for": "0m"},
        "PTFAdminDependencyMapMiss": {"severity": "warning", "for": "5m"},
        "PTFAdminDependencyTimeoutRate": {"severity": "warning", "for": "5m"},
        "PTFAdminDependencyFailureRate": {"severity": "critical", "for": "5m"},
        "PTFAdminDependencyClientErrorRate": {"severity": "warning", "for": "5m"},
        "PTFAdminRetryStorm": {"severity": "warning", "for": "5m"},
        "PTFAdminDependencyLatencyP95": {"severity": "warning", "for": "5m"},
        "PTFAdminDependencyCircuitOpenRate": {"severity": "critical", "for": "2m"},
    }

    def _get_dep_health_rules(self, alert_rules):
        for g in alert_rules["spec"]["groups"]:
            if g["name"] == "ptf-admin-dependency-health":
                return g["rules"]
        return []
    ...
    def _get_rule(self, alert_rules, name):
        for r in self._get_dep_health_rules(alert_rules):
            if r["alert"] == name:
                return r
        return None
    ...
    @pytest.mark.parametrize("alert_name,expected", list(EXPECTED_ALERTS.items()))
    def test_alert_severity_and_for(self, alert_rules, alert_name, expected):
        """Each dependency-health alert has correct severity and for duration."""
        rule = self._get_rule(alert_rules, alert_name)
        assert rule is not None, f"Alert '{alert_name}' not found in dependency-health group"
        assert rule["labels"]["severity"] == expected["severity"]
        assert rule["for"] == expected["for"]

    @pytest.mark.parametrize("alert_name", list(EXPECTED_ALERTS.keys()))
    def test_alert_labels_complete(self, alert_rules, alert_name):
        """Each dependency-health alert has required labels."""
        rule = self._get_rule(alert_rules, alert_name)
        assert rule is not None
        missing = self.REQUIRED_LABELS - rule.get("labels", {}).keys()
        assert not missing, f"{alert_name} missing labels: {missing}"
    ...
    @pytest.mark.parametrize("alert_name", list(EXPECTED_ALERTS.keys()))
    def test_alert_annotations_complete(self, alert_rules, alert_name):
        """Each dependency-health alert has required annotations."""
        rule = self._get_rule(alert_rules, alert_name)
        assert rule is not None
        missing = self.REQUIRED_ANNOTATIONS - rule.get("annotations", {}).keys()
        assert not missing, f"{alert_name} missing annotations: {missing}"
    ...
    @pytest.mark.parametrize("alert_name", list(EXPECTED_ALERTS.keys()))
    def test_alert_service_label(self, alert_rules, alert_name):
        """All dependency-health alerts have service=ptf-admin."""
        rule = self._get_rule(alert_rules, alert_name)
        assert rule is not None
        assert rule["labels"]["service"] == "ptf-admin"
    ...
    @pytest.mark.parametrize("alert_name", list(EXPECTED_ALERTS.keys()))
    def test_alert_runbook_url_has_anchor(self, alert_rules, alert_name):
        """Each dependency-health alert runbook_url has a # anchor."""
        rule = self._get_rule(alert_rules, alert_name)
        assert rule is not None
        url = rule["annotations"]["runbook_url"]
        assert "#" in url, f"{alert_name} runbook_url has no anchor: {url}"
...
class TestDependencyHealthAlertExpressions:
    """Verify specific PromQL expressions for dependency-health alerts."""
    ...
    def test_guard_failopen_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminGuardFailOpen")
        assert "ptf_admin_guard_failopen_total" in rule["expr"]

    def test_dependency_map_miss_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyMapMiss")
        assert "ptf_admin_dependency_map_miss_total" in rule["expr"]

    def test_dependency_timeout_rate_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyTimeoutRate")
        assert "ptf_admin_dependency_call_total" in rule["expr"]
        assert 'outcome="timeout"' in rule["expr"]
        assert "0.02" in rule["expr"]

    def test_dependency_failure_rate_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyFailureRate")
        assert "ptf_admin_dependency_call_total" in rule["expr"]
        assert 'outcome="failure"' in rule["expr"]
        assert "0.01" in rule["expr"]

    def test_dependency_client_error_rate_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyClientErrorRate")
        assert "ptf_admin_dependency_call_total" in rule["expr"]
        assert 'outcome="client_error"' in rule["expr"]
        assert "0.05" in rule["expr"]

    def test_retry_storm_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminRetryStorm")
        assert "ptf_admin_dependency_retry_total" in rule["expr"]
        assert "ptf_admin_dependency_call_total" in rule["expr"]
        assert "0.2" in rule["expr"]

    def test_dependency_latency_p95_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyLatencyP95")
        assert "histogram_quantile" in rule["expr"]
        assert "ptf_admin_dependency_call_duration_seconds_bucket" in rule["expr"]
        assert "0.8" in rule["expr"]

    def test_dependency_circuit_open_rate_expr(self, alert_rules):
        rule = self._get_rule(alert_rules, "PTFAdminDependencyCircuitOpenRate")
        assert "ptf_admin_dependency_call_total" in rule["expr"]
        assert 'outcome="circuit_open"' in rule["expr"]
